...
unsigned long long serie_fibo_long_long_u(unsigned long long n)
{
	if (n == 0)
	{
		return 0;
...
long double serie_fibo_double_u(long double n)
{
	if (n == 0)
	{
		return 0;
...
unsigned char serie_fibo_char_u(unsigned char n)
{
	if (n == 0)
	{
		return 0;
...
short serie_fibo_short(short n)
{
	if (n == 0)
	{
		return 0;
...
void num_max()
{
	...
	short sh = 0;
	while(fibo_short(++sh) >= fibo_short(sh-1));
	printf("short %d\n", sh-1);
	//serie_fibo_short(sh-1);
...
void num_max_unsigned()
{
	...
	//serie_fibo_long_long_u(lli-1);

	long double ldb = 0.0;
	while(fibo_double_u(++ldb) <= pow(2,sizeof(long double)*8));
	printf("long double %Lf\n", ldb-1.0);
	//serie_fibo_double_u(ldb-1.0);
...
int main()
{
	...
	//printf("%d\n", (int)fibo_char_u(200));
	printf("Valor maximo que puede llegar la sucesion de Fibonacci para cada tipo de dato Signed\n");
	num_max();
	printf("Valor maximo que puede llegar la sucesion de Fibonacci para cada tipo de dato Unsigned\n");
	num_max_unsigned();
	return 0;
}
